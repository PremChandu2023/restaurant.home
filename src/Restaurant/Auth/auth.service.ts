...
export class AuthService {
    ...
    constructor(
        ...
        @InjectRepository(Roles) private rolesRespository: Repository<Roles>,
    ...
    async authenticateLogin(createLogin: loginEmployeeDto) {
        try {
            ...
            const employee = await this.employeeRespository.findOne({
                where: { email: createLogin.email },
                relations: { tokens: false },
            });
            if (!employee) {
                ...
                if (
                    await this.verifyPassword(createLogin.password, employee.password)
                ) {
    ...
    async generateRefreshToken(employee: Employee) {
        this.logger.log('Generation of refresh token');
        ...
        const token = await this.jwtService.signAsync(payload, {
            expiresIn: '7d',
            secret: secretkey,
        }); // 7 * 24 * 60 * 60 * 1000 calculates the total number of milliseconds in 7 days.
    ...
    async validateRefereshToken(refereshToken: string) {
        ...
        const token = await this.jwtService.verifyAsync(refereshToken, {
            secret: 'employeekey',
        });
        const referenceToken = await this.tokenRepository.findOne({
            where: { token_value: refereshToken },
        });
        if (!referenceToken) {
    ...
    async registerEmployee(employee: registerEmployeeDto) {
        ...
        const newEmployee = await this.employeeRespository.findOne({
        ...
        if (newEmployee) {
            ...
            const Role = await this.rolesRespository.findOne({
    ...
    async findEmployeeDetails(id: number) {
        ...
        const result = await this.employeeRespository.findOne({
            where: {
                id: id,
            },
            select: [
        ...
        this.logger.log(
            `Completed the process of Fetching employee details for ID: ${id}`,
        );
        return result;
    }
    async createRoles(roleName: createRoleDto) {
        this.logger.log('Initializes the process of creating new role');
        const newRoles = await this.rolesRespository.findOne({
        ...
        const newRole = this.rolesRespository.create({
    ...
    async updateRoles(updateRole: updateRoleDto, employeid: number) {
        ...
        newEmployee.roles = Role;
        this.logger.log(
            `Procees of updating the roles of employeeid1${employeid} has been completed`,
        );
