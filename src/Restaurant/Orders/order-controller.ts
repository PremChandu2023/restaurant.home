...
export class OrderController {
      ...
      switch (error) {
        ...
        default:
          throw new InternalServerErrorException({
            message: 'Cannot get the order details of the particular id',
          });
      }
    }
  }
  ...
  async updateOrderQuantity(
    ...
    try {
      return await this.orderService.updateOrderQuantity(
        updateOrder,
        orderItemId,
      );
    } catch (error) {
  ...
  async updatePaymentStatus(@Param('id') id: number) {
    try {
      ...
      return await this.orderService.updatePaymentandOrderStatus(
        updateStatus,
        id,
      );
    } catch (error) {
