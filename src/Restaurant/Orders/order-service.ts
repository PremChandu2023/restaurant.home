...
export class OrderServices {
  ...
  async createOrder(createOrder: createOrderDTo, id: number) {
    ...
    for (const OrderItemdata of createOrder.items) {
      ...
      const menuItem = await this.menuItemRespository.findOne({
  ...
  async getOrderById(OrderId: number) {
    this.logger.log(
      `Initializes the process of fetching Order by id ${OrderId}`,
    );
  ...
  async getBillById(OrderId: number) {
    this.logger.log(
      `Initializes the process of creating a new Bill by using given ${OrderId}`,
    );
    const newOrder = await this.orderRespository.findOne({
      where: {
        order_id: OrderId,
    ...
    const totalPrice = await this.calculatePrice(newOrderItems);
  ...
  async deleteMenuItem(orderItemId: number) {
    this.logger.log(
      `Deleting the MenuItem with given orderItemId ${orderItemId}`,
    );
    const newOrderItem = await this.orderItemRespository.findOne({
  ...
  async updatePaymentandOrderStatus(
    updateBody: updatePaymentDTo,
    orderId: number,
  ) {
    this.logger.log(
    ...
    return this.datasource.transaction(async (manager) => {
      ...
      const result = await manager
        ...
        .update(Order)
        .set({
          paymentStatus: updateBody.orderStatus,
  ...
  /**
   * Operations where the database operations require inner joins
   */
